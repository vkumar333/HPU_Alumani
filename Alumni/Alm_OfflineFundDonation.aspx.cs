...
public partial class Alumni_Alm_OfflineFundDonation : System.Web.UI.Page
{
    ...
    public DataSet Getmain()
    {
        ...
        dr["Draftno"] = TxtDraft.Text.ToString();
        dr["paymentMode"] = "OFFLINE";
    ...
    //        names.Add("@Draftno"); values.Add(Draftno); types.Add(SqlDbType.VarChar); size.Add("MAX"); outtype.Add("false");
    //        names.Add("@paymentMode"); values.Add(paymentMode); types.Add(SqlDbType.VarChar); size.Add("MAX"); outtype.Add("false");
    ...
    public int insertContributorFundData(ref string Message, ref ArrayList Result)
    {
        try
        {
            ...
            names.Add("@paymentMode"); values.Add(paymentMode); types.Add(SqlDbType.VarChar); size.Add("MAX"); outtype.Add("false");
            names.Add("@Fk_contribution_ID"); values.Add(ViewState["funddetails"]); types.Add(SqlDbType.Int); size.Add("MAX"); outtype.Add("false");
            names.Add("@paymenttype"); values.Add(paymenttype); types.Add(SqlDbType.Int); size.Add("2"); outtype.Add("false");
            names.Add("@Receiverbank"); values.Add(Receiverbank); types.Add(SqlDbType.VarChar); size.Add("20"); outtype.Add("false");
            names.Add("@paymentDate"); values.Add(paymentDate); types.Add(SqlDbType.VarChar); size.Add("20"); outtype.Add("false");
            names.Add("@CountryCode"); values.Add(Countrycode); types.Add(SqlDbType.VarChar); size.Add("10"); outtype.Add("false");
            names.Add("@Anonymous"); values.Add(Anonymous); types.Add(SqlDbType.Int); size.Add("10"); outtype.Add("false");
            names.Add("@FK_countryID"); values.Add(FkCountyID); types.Add(SqlDbType.Int); size.Add("10"); outtype.Add("false");
            names.Add("@pk_FundId"); values.Add(regId); types.Add(SqlDbType.VarChar); size.Add("10"); outtype.Add("true");

            if (Dobj.ExecuteTransactionMsgIO("ALM_Contributor_Fund_Data_Insert", values, names, types, size, outtype, ref Message, ref Result) > 0)
            {
                Message = Dobj.ShowMessage("S", "");
                return 1;
            }
    ...
    public bool Validation()
    {
        ...
        else if (mobile.Text.Trim() != "" && mobile.Text.Length < 10)
        {
            ClientMessaging("Invalid Mobile Number!.");
            mobile.Focus();
            return false;
        }
        if (string.IsNullOrEmpty(TxtDraft.Text.ToString()))
        {
            ClientMessaging("Reference/Draft No can not be blank!.");
            TxtDraft.Focus();
            return false;
        }
        return true;
    ...
    public void Save()
    {
        ...
        if (Validation())
        {
            try
            {
                ...
                @Draftno = TxtDraft.Text.ToString();
                @paymentMode = "OFFLINE";
                paymentDate = Txt_paymentDate.Text.ToString();
                paymenttype = Convert.ToInt32(ddl_PaymentType.SelectedIndex);
                Receiverbank = TxtReciverBankAcc.Text.ToString();
                Countrycode = Convert.ToInt32(ddlCountry.SelectedIndex);
                FkCountyID = Convert.ToInt32(ddlCountry.SelectedIndex);
                Anonymous = Convert.ToInt32(exampleCheck1.Checked);
                ...
                if (insertContributorFundData(ref Message, ref Result) > 0)
                {
                    if (Result.Count > 0)
                    {
                        ...
                        Page.ClientScript.RegisterStartupScript(this.GetType(), "HidePopup", "<script>document.getElementById('contribute').style.display = 'none';</script>");
                        Page.ClientScript.RegisterStartupScript(this.GetType(), "CloseModalAndUnfade", @"   <script>                       
                        $('#contribute').modal('hide');                     
                        $('body').removeClass('modal-open');
                        $('.modal-backdrop').remove();
                        $('body').css('padding-right', '');
                        $('body').css('overflow', 'auto');    </script>");
                    }
                }
                else
                {
    ...
    protected void btnPay()
    {
        ...
        try
        {
            //if (Session["ContributorAmt"] == null)
            //{
            //    lblPaymentMsg.Text = "Contribution amount is not available. Please Try Again or contact to University!";
            //    return;
            //}
            string Message = "";
            ArrayList Result = new ArrayList();
            ...
            //if (insertContributionPaymentRecord(ref Message, ref Result) > 0)
            //{
            //    if (Result.Count > 0)
            //    {
            //        Session["Pk_purchaseid"] = Result[1].ToString().Trim();
            //        Session["temp_Pk_purchaseid"] = Result[1].ToString().Trim();
            //        //Response.Redirect("../Onlinepayment/ALM_Contribution_PaymentGateway.aspx", false);
            //        SentToSbi();
            //    }
            //}
            //else
    ...
    private void SentToSbi()
    {
        try
        {
            ...
            string amount = "1";//Convert.ToDecimal(ds.Tables[0].Rows[0]["Adm_RegFees"]).ToString("g29");
    ...
    protected void gvConPayHistry_RowCommand(object sender, GridViewCommandEventArgs e)
    {
