...
public partial class Alumni_Alm_Alumni_Show_Alumni_Profile_Search : System.Web.UI.Page
{
    ...
    private void contactRepeter(int alumniID)
    {
        ...
        if (!dtContact.Columns.Contains("encId"))
            dtContact.Columns.Add("encId", typeof(string));
        ...
        //string rawUrl = "http://localhost:53991/Alumni/Alm_Alumni_Show_Alumni_Profile_Search.aspx?ID=" + encId;
        //string rawUrl = "https://alumni.hpushimla.in/alumni/Alm_Alumni_Show_Alumni_Profile_Search.aspx?ID=" + encId;
        //string rawUrl = "http://localhost:53991/Alumni/" + "Alm_Alumni_Show_Alumni_Profile_Search.aspx" + "?ID=" + encId;
    ...
    private void membershipRepeter(int alumniID)
    {
        ...
        // ✅ Ensure column exists (and not duplicated)
        if (!dtMembership.Columns.Contains("encId"))
            dtMembership.Columns.Add("encId", typeof(string));
        ...
        if (dtMembership.Rows.Count > 0)
        {
            for (int x = 0; x < dtMembership.Rows.Count; x++)
            {
                //string encId = crp.EncodeString(Convert.ToInt32(alumniID));
                //string shortUrl = AlumniUrlShortener.GetShortUrl(longUrl);
                dtMembership.Rows[x]["encId"] = encId;
            }
    ...
    protected void lnkFullProfileDetails_Click(object sender, EventArgs e)
    {
        try
        {
            Anthem.LinkButton lnkBtn = (Anthem.LinkButton)sender;
            string encId = lnkBtn.CommandArgument.ToString();

            string script = string.Format("window.open('../Alumni/ALM_AlumniProfile.aspx?ID=" + Server.UrlEncode(encId) + "', 'winname', 'directories=no, titlebar=no, toolbar=no, location=no, status=no, menubar=no, addressbar=no, scrollbars=yes, resizable=yes, width=850');", "");
            Anthem.Manager.IncludePageScripts = true;
