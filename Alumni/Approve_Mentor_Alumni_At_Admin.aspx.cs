...
public partial class Alumni_Approve_Mentor_Alumni_At_Admin : System.Web.UI.Page
{
    ...
    {
        SubSonic.StoredProcedure sp = new SubSonic.StoredProcedure("ALM_AlumniMentorReqAprroval", DataService.GetInstance("IUMSNXG"), "");
        return sp;
    }
    ...
    protected void almRequest_Action()
    {
        ...
        if (ds.Tables[0].Rows.Count > 0)
        {
            ...
            {
                string pkid = ds.Tables[0].Rows[x]["pk_alumniid"].ToString();
    ...
    public DataSet GetPendingMentorAlumini()
    {
        try
        {
            ClearArrayLists();
            return DAobj.GetDataSet("ALM_Pending_Mentor_Alumni_Excel", values, names, types);
        }
        catch { throw; }
    }
    ...
    void ExportGridToExcel(DataSet ds)
    {
        ...
        int row;
        string strFile = "List Of Pending Mentor Alumni Report.xls";
        ...
        headerCell.SetCellValue("Registration No.");
        xlsWorksheet.SetColumnWidth(1, "Registration No.".ToString().Length * 300);
        headerCell.CellStyle = headerCellStyle_font_Bold;
        headerCell.CellStyle.Alignment = 1;
    ...
    public DataSet GetApprovedMentorAlumni()
    {
        try
        {
            ClearArrayLists();
            return DAobj.GetDataSet("ALM_Approved_Mentor_Alumni_Excel", values, names, types);
        }
        catch { throw; }
    }
    ...
    void ExportApprovedGridToExcel(DataSet ds)
    {
        ...
        boldFont.Boldweight = NPOI.HSSF.UserModel.HSSFFont.BOLDWEIGHT_BOLD;
        int row;
        string strFile = "List Of Approved Mentor Alumni Report.xls";
        string filename = Server.MapPath("~/XLSFiles/" + strFile);
        string path = Server.MapPath("~/Images/hpu-logo.jpg");
    ...
    protected void gvpendinreq_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            DataRowView rowView = (DataRowView)e.Row.DataItem;
            string alumnitype = rowView["alumnitype"].ToString();

            if (alumnitype == "ExStu" || alumnitype == "S")
            {
                e.Row.Enabled = false;
            }
        }
