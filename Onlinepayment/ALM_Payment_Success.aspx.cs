...
public partial class Onlinepayment_ALM_Payment_Success : System.Web.UI.Page
{
    ...
    protected void btnBack_Click(object sender, EventArgs e)
    {
        try
        {
            Response.Redirect("../Onlinepayment/ALM_Common_PaymentGateway.aspx");
        }
        catch (Exception ex)
        {
            string Msg = "";
    ...
    protected void btnFailed_Click(object sender, EventArgs e)
    {
        try
        {
            Response.Redirect("../Alumin_Loginpage.aspx");//("../Onlinepayment/ALM_Common_PaymentGateway.aspx");
        }
        catch (Exception ex)
        {
            string Msg = "";
            ...
            else
            {
                ...
                if (transaction_id.Length > 0)
                {
                    ...
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        Session["fk_regid"] = ds.Tables[0].Rows[0]["fk_regid"].ToString();
                        ...
                        Session["Pk_TranId"] = ds.Tables[0].Rows[0]["Pk_TranId"].ToString();
                        Session["Userid"] = ds.Tables[0].Rows[0]["userid"].ToString();
                        Session["password"] = ds.Tables[0].Rows[0]["password"].ToString();
                        Session["EmailID"] = ds.Tables[0].Rows[0]["Email"].ToString();
                        Session["Alumni_Name"] = ds.Tables[0].Rows[0]["alumni_name"].ToString();
                        Session["MobileNo"] = ds.Tables[0].Rows[0]["contactno"].ToString();
                    }
                    else
                    {
                ...
                if (status.ToUpper() == "SUCCESS")
                {
                    ...
                    sendSMS(Session["MobileNo"].ToString());
                    ...
                    //if (mobileno != "")
                    //{
                    //    string message = "Dear " + Session["Alumni_Name"] + ", ";
                    //    message += "Your Alumni Registration has been done successfully, Please find your User Name & Password below:-" + "User Name - " + Session["EmailID"] + " & Password - " + Session["password"] + "";
                    //    message += "Note:- Your login details have been sent on entered email id & phone no";
                    //    message += Environment.NewLine;
                    //    message += "HPU";
                    //    sendSMS(mobileno, message);
                    //}

                    updateAlumniApproved(Convert.ToInt32(Session["fk_regid"].ToString()));
                ...
                if (Session["Pk_TranId"].ToString() == transaction_id)  //check here our unique pk_transid with responce trans id as refno
                {
                    Pk_TranId = Session["Pk_TranId"].ToString();
                    IpAddress = HttpContext.Current.Request.UserHostAddress.ToString();
                ...
                strMsg.Append(HeaderMsg);
                strMsg.Append(" Details are given below:<br />");
                strMsg.Append("Transcation id :-  " + Session["Pk_TranId"].ToString() + "  <br />");
    ...
    public DataSet Get_All_Details_of_Current_Trasaction(string PK_TranId)
    {
        ...
        return DAobj.GetDataSet("HPU_ALM_Select_All_Details_ToLogin_After_SBI_Responce", values, names, types);
    }

    public DataSet Alumni_Approved_After_PaymentSuccess(int alumniId)
    {
        Clear();
        names.Add("@pk_alumniId"); values.Add(alumniId); types.Add(SqlDbType.Int);
        return DAobj.GetDataSet("ALM_Approved_After_PaymentSuccess_Without_AdminApproval", values, names, types);
    }

    //public int Alumni_Approved_After_PaymentSuccess(int alumniId, ref string Message)
    //{
    //    Clear();
    //    names.Add("@pk_alumniId"); values.Add(alumniId); types.Add(SqlDbType.Int);
    //    if (DAobj.ExecuteTransactionMsg("ALM_Approved_After_PaymentSuccess_Without_AdminApproval", values, names, types, ref Message) > 0)
    //    {
    //        Message = DAobj.ShowMessage("U", "");
    //        return 1;
    //    }
    ...
    //    catch (Exception ex)
    //    {
    //        lblSuccessMsg.Text = ex.Message.ToString();
    //    }
    //}
    ...
    private void sendSMS(string mobiles)
    {
        try
        {
            string smsMessage = "Dear Visitor, Congratulations! You have been successfully registered in the alumni portal of Himachal Pradesh University, Shimla. Your credentials are mentioned as under: ID : " + Session["EmailID"].ToString() + " Password : " + Session["password"].ToString() + ".  HPU";
    ...
    protected void updateAlumniApproved(int pk_AlumiId)
    {
        try
        {
            DataSet ds = Alumni_Approved_After_PaymentSuccess(pk_AlumiId);
            if (ds == null)
            {
